...
struct AccessField<T, QVecSize, MemoryLayout, collisionType, Esotwist> {
    ...
    inline static void writeMoments(ComputeUnitForcing<T, QVecSize, MemoryLayout, collisionType, Esotwist> &cu, QVec<T, QVecSize> &q, tNi i, tNi j, tNi k) {
        if (cu.evenStep) {
            ...
            cu.Q[cu.dirnQ11(i,j,k)].q[Q12] = q[Q12];
            cu.Q[cu.dirnQ09(i,j,k)].q[Q10] = q[Q10];
            cu.Q[cu.dirnQ07(i,j,k)].q[Q08] = q[Q08];
        } else { // odd step
            tNi ind0 = cu.index(i,j,k);
            ...
            cu.Q[cu.dirnQ11(i,j,k)].q[Q11] = q[Q12];
            cu.Q[cu.dirnQ09(i,j,k)].q[Q09] = q[Q10];
            cu.Q[cu.dirnQ07(i,j,k)].q[Q07] = q[Q08];
        }
    }
};
